...
public class ServicesInventory {
	...
	public ServicesInventory(int currentUserID) {
		...
		try{
		    ...
		    while( rs.next() ){
		    	...
		    	item[rs.getInt("service_id")-1] = rs.getString("service_active");   
		    	System.out.println(item[rs.getInt("service_id")-1]);		
		    }
	...
	public void store(ServicesInventory servicesInventory){
		...
		try{
			...
			for(i=0,j=1;i<8;i++,j++){
				...
				rs = st.executeQuery("select * from user_services where user_id =" + this.getUserID() + " and service_id ="+j);
				count = 0;
				while(rs.next()){
					count++;
				}
				if(count > 0){
					st.execute("update user_services set service_active = '"+item[i]+"' where user_id = "+ this.getUserID() +" and service_id =" +j);
				}else{
					st.execute("insert into user_services(user_id, service_id, service_active) values ("+ this.getUserID() +", "+ j +", '"+ item[i] +"')");
				}
			}
