...
public class VideoLibEdit extends SelectorComposer<Component>  {

	private static final long serialVersionUID = 1L;

	private SingleConnect singleConnect = new SingleConnect();
	
	private VideoLibEditInventory videoLibEditInventory;
		...
		if(this.getUserID() == null){
			...
			try{
				...
				if(this.getMightyUserID().equals("149565")){
					...
					videoLibEditInventory = new VideoLibEditInventory(this.getUserID(), this.getVideoID());
				}else{
					System.out.println("Mighty Session ID:" + this.getMightyUserID());
	...
	public VideoLibEditInventory getVideoLibEditInventory() {
		return videoLibEditInventory;
	}
	...
	@Wire
	private Textbox textOne, textTwo, textThree, textFour;
	...
	public void submitData(){
		try{
			...
			videoLibEditInventory.setVideoTitle(videoTitle);
			videoLibEditInventory.setVideoURL(videoURL);
			videoLibEditInventory.setVideoDescription(videoDescription);
			videoLibEditInventory.setVideoID(this.getVideoID());
			
			videoLibEditInventory.store(videoLibEditInventory);
