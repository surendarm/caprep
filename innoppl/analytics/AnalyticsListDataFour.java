...
public class AnalyticsListDataFour {
	...
	private List<AnalyticsList> blockFiveReportData = new ArrayList<AnalyticsList>();
	private Integer userID=0;

	private String blockOneTitle, blockTwoTitle, blockThreeTitle, blockFourZeroTitle, blockFourOneTitle, blockFiveTitle, periodValue, totalSubmissions;

	private String legendOne, legendTwo, legendThree; 


	@SuppressWarnings("resource")
	public AnalyticsListDataFour(){
		...
		int[] blockOnePositions = new int[]{7,8,9,10};
		int[] blockTwoPositions = new int[]{14, 15, 16, 17, 18, 19, 20};
		int[] blockThreePositions = new int[]{22, 23, 24, 25, 26};
		int[] blockFourZeroPositions = new int[]{32, 34, 36, 38};
		int[] blockFourOnePositions = new int[]{41, 42, 43, 44};
		int[] blockFivePositions = new int[]{62, 63, 64, 65};
		...
		try{
			...
			rs = st.executeQuery("select user_form_id from user_forms where user_id = "+this.getUserID()+" and form_name in ('104 form','104 Form')");
			while(rs.next()){
			...
			if(startDate != null && !startDate.equals("") && endDate != null && !endDate.equals("")){
				rs = st.executeQuery("select * from form_submission_index where user_id = "+this.getUserID()+" and form_name in ('104 form','104 Form') and created_date between '"+ startDate +"' and '" +endDate+"' order by form_submission_id desc ");

			}else{
				rs = st.executeQuery("select * from form_submission_index where user_id = "+this.getUserID()+" and form_name in ('104 form','104 Form')  and created_date = '"+ todaysDate +"' order by form_submission_id desc ");
			...
			while(rs.next()){
				for(i=0;i<blockFivePositions.length;i++){
					...
					while( innerRs.next() ){
						optionChosen = innerRs.getString("option_description");
						if(optionChosen.equals("1")){
							oneC[blockFivePositions[i]]++;
						}else if(optionChosen.equals("2")){
							twoC[blockFivePositions[i]]++;
						}else if(optionChosen.equals("3")){
							threeC[blockFivePositions[i]]++;
						}
					}
				}
			}	
			...
			for(i=0;i<blockFivePositions.length;i++){
				rs= st.executeQuery("select distinct(c.component_id), c.component_name, sc.label, op.option_description," +
						...
						"left join sub_components_options sco on sc.sub_component_id = sco.sub_component_id " +
						"left join options op on sco.option_id = op.option_id " +
						"where uc.user_form_id = "+ userFormID +"and position_id="+blockFivePositions[i]);

				while( rs.next() ){
					componentName =  rs.getString("component_name");
				}

				blockFiveReportData.add(new AnalyticsList(blockFivePositions[i], componentName, oneC[blockFivePositions[i]], twoC[blockFivePositions[i]], threeC[blockFivePositions[i]]));
			}
			...
			rs= st.executeQuery("select distinct(c.component_id), c.component_name, sc.label, op.option_description," +
					...
					"left join sub_components_options sco on sc.sub_component_id = sco.sub_component_id " +
					"left join options op on sco.option_id = op.option_id " +
					"where uc.user_form_id = "+ userFormID +"and position_id in(13)");
			...
			rs= st.executeQuery("select distinct(c.component_id), c.component_name, sc.label, op.option_description," +
					...
					"left join sub_components_options sco on sc.sub_component_id = sco.sub_component_id " +
					"left join options op on sco.option_id = op.option_id " +
					"where uc.user_form_id = "+ userFormID +"and position_id in(21)");
			...
			rs= st.executeQuery("select distinct(c.component_id), c.component_name, sc.label, op.option_description," +
					...
					"left join sub_components_options sco on sc.sub_component_id = sco.sub_component_id " +
					"left join options op on sco.option_id = op.option_id " +
					"where uc.user_form_id = "+ userFormID +"and position_id in(28)");
			...
			rs= st.executeQuery("select distinct(c.component_id), c.component_name, sc.label, op.option_description," +
					...
					"left join sub_components_options sco on sc.sub_component_id = sco.sub_component_id " +
					"left join options op on sco.option_id = op.option_id " +
					"where uc.user_form_id = "+ userFormID +"and position_id in(57)");

			while( rs.next() ){
				this.blockFiveTitle= rs.getString("component_name");
			}
	...
	public List<AnalyticsList> getBlockFiveReports() {
		return blockFiveReportData;
	}
	...
	public String getBlockFiveTitle() {
		return blockFiveTitle;
	}




	public void setBlockFiveTitle(String blockFiveTitle) {
		this.blockFiveTitle = blockFiveTitle;
	}
